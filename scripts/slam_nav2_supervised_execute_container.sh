...
supervised path cap cannot exceed 1.40 m. Runtime cannot exceed 80 s.
EOF
}
...
if not 0.0 < runtime_s <= 80.0:
    raise SystemExit("--max-runtime-s must be in (0, 80] for supervised motion")
