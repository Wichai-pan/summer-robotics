...
supervised path cap cannot exceed 1.40 m. Runtime cannot exceed 50 s.
EOF
}
...
if not 0.0 < runtime_s <= 50.0:
    raise SystemExit("--max-runtime-s must be in (0, 50] for supervised motion")
