...
STAGE_MARKERS = (
    ...
    ("[1/3] Gemini -> small-cup grasp pose", TaskState.SET_GRASP_CAMERA),
    ("[2/3] ACT small-cup Pick", TaskState.GRASPING),
    ("SMALL_CUP_PICK_READY", TaskState.HOLDING),
    ("[2/4] Set Gemini navigation pose and move table -> sofa", TaskState.NAVIGATING),
    ("[1/2] Gemini -> navigation pose", TaskState.SET_MAPPING_CAMERA),
    ("[2/2] Table workspace -> sofa workspace", TaskState.NAVIGATING),
    ("SMALL_CUP_AT_SOFA", TaskState.HOLDING),
    ("[3/4] Set Gemini navigation pose and move sofa -> table", TaskState.NAVIGATING),
    ("[2/2] Sofa workspace -> table workspace", TaskState.NAVIGATING),
    ("SMALL_CUP_AT_TABLE", TaskState.HOLDING),
    ("[4/4] Set Gemini place pose and run Place", TaskState.PLACING),
    ("[1/2] Gemini -> small-cup place pose", TaskState.SET_GRASP_CAMERA),
    ("[2/2] ACT small-cup Place", TaskState.PLACING),
