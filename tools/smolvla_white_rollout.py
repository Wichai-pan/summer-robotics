#!/usr/bin/env python3
"""Run a short, supervised SmolVLA action chunk on the physical white arm.

This is the deployment gate after the single-action smoke test.  One live
Gemini+wrist observation produces a SmolVLA chunk; only the first ``--steps``
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "continue a later SmolVLA chunk from the current pose instead of requiring "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "does not change the SmolVLA observation vector"
        ),
    )
...
def summarize_policy_chunks(
    ...
    """Summarize each SmolVLA replan window without changing rollout behavior."""
    if n_action_steps <= 0:
...
def calibration_differences(
    expected: dict[str, object], actual: dict[str, object]
) -> dict[str, object]:
    """Return human-readable homing/range differences without writing motors."""
...
def verify_calibration_read_only(
    robot: object, attempts: int = 3, delay_s: float = 0.15
) -> dict[str, object]:
    """Retry transient serial reads, returning the final mismatch details."""
    final: dict[str, object] = {}
    for attempt in range(attempts):
        try:
            actual = robot.bus.read_calibration()
            final = calibration_differences(robot.calibration, actual)
        except Exception as exc:  # The final error is reported, never ignored.
            final = {"_read_error": f"{type(exc).__name__}: {exc}"}
        if not final:
            return {}
...
def main() -> int:
    ...
    try:
        ...
        calibration_error = verify_calibration_read_only(robot)
        if calibration_error:
            raise RuntimeError(
                f"white motor registers do not match {args.white_id}: "
                + json.dumps(calibration_error, sort_keys=True)
            )
        ...
        print("SmolVLA 短 rollout 已准备；目前仍为松扭矩。")
        print(
            json.dumps(
                {
                    "steps": args.steps,
        ...
        if os.environ.get("FORESTBRIDGE_DEMO_ARMED") == "1":
            print(f"AUTO_PIPELINE 已授权；自动确认 {confirmation} SmolVLA 动作。")
        elif input(
