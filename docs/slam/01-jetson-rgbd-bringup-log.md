...
内部 TF 存在。基于 Gemini 工厂标定以及已有 IK/ACT 实机证据，本阶段不新增深度
已知距离复测或相机重标定门槛。预检只需关闭以下可自动复核的链路问题：

1. 统计 RGB 与对齐 Depth 的时间戳差；
2. 统计持续帧率和掉帧，而不只使用消息总数估算；
3. 确认 ROS graph 中只有一个相机驱动节点拥有这些 topic。
...
确认。结合 IK 阶段的毫米级目标重复性、ACT 固定场景真机抓取和 Gemini 厂家
标定，本项目接受现有相机能力为 SLAM 基线，不再新增已知距离复测或重新标定
门槛。因此相机链路足以进入 camera-only 静止视觉里程计。
...
8. 现有 IK 外参拟合中的 93.4 mm 留出误差仍不能作为正式 TF；该限制针对那份
   非刚性标记拟合结果，不代表 Gemini 深度能力失效，也不要求重新标定相机。
...
不要求先发布 `base_link -> camera_link`。移动建图前优先整理和转换已有 IK/
安装测量数据，不重新启动完整标定流程。
