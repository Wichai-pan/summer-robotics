...
- [ ] Fix Gemini exposure, RGB resolution, FPS and camera pose. Log them.
...
- [ ] Start from the same safe reset pose and jar start-cell for every pilot.
- [ ] Record the full episode: pre-grasp, approach, close, lift, hold and
  return/reset. Do not begin recording halfway through a grasp.
...
- [ ] Required keys exist in every frame: `observation.state`, `action`, and
