...
def _resolve_port(serial: str, fallback: str) -> str:
    """按电机控制板的 USB 序列号解析 Mac 端口（端口名会变，序列号固定）。"""
...
class XLerobotConfig(RobotConfig):
    ...
    port1: str = field(default_factory=lambda: _resolve_port("5B3D043224", "/dev/ttyACM0"))  # 黑板: 左臂(1-6)+头(7,8)
    port2: str = field(default_factory=lambda: _resolve_port("5B3D040988", "/dev/ttyACM1"))  # 白板: 右臂(1-6)+底盘(7,8,9)
